...
export function renderLegend(el: HTMLElement): void {
  const rows: string[] = [];
  rows.push(`<div style="font-weight:600;margin-bottom:6px">Change in labor force over past 20 years</div>`);
  for (const b of BUCKETS) {
    rows.push(
      `<div style="display:flex;align-items:center;gap:6px;margin:2px 0">
         <span style="width:14px;height:14px;background:${b.color};display:inline-block"></span>
         <span>${b.label} <span style="color:#a8a8a8">(${rangeLabel(b.lower, b.upper)})</span></span>
       </div>`,
    );
  }
  rows.push(
    `<div style="display:flex;align-items:center;gap:6px;margin-top:6px">
       ...
       <span style="color:#a8a8a8">${NO_DATA_LABEL}</span>
     </div>`,
  );
